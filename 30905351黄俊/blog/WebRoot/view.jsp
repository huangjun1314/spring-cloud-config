...
<html>
  ...
  <script type="text/javascript">
  	...
  	function deleted(id){
  		$.ajax('delete','id='+id,function(data){
  			if(data>0){
  				$("#nn"+id).del();
  			}
  		});
  ...
  <body>
    <div align="center">
    	<form action="${pageContext.request.ContextPath}/view" method="post">
    		<table border="1px">
    			<tr>
    				<td>用户名</td><td>昵称</td><td>手机</td>
    				<td>地址</td><td>操作</td>
    			</tr>
    			<c:forEach var="temp" items="${usersList}">
    				<tr id="nn${temp.id}">
    					<td>${temp.username}</td><td>${temp.nickname}</td><td>${temp.mobile}</td>
    					<td>${temp.address}</td><td><a href="deleted(${temp.id})">删除</a></td>
    				</tr>
    			</c:forEach>
    		</table>
    	</form>
    	...
    	<div><input type="button" value="添加数据" onclick="location='${pageContext.request.ContextPath}/addview'"></div>
